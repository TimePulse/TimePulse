...
describe LogicalAuthz::Helper do
  before do
    @group = Factory(:group)
    Factory(:permission, :group => @group, :controller => "projects")
    Factory(:permission, :group => @group, :controller => "work_units", :action => "edit", :subject_id => 1)
  end
  ...
  describe "authorized" do
    ...
    describe "should recognize authorized users" do
      before do
        user = Factory(:user, :groups => [@group])
        activate_and_login(user)
        Factory(:permission, :group => @group, :controller => "clients", :action => "show")
      end
